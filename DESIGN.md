...
The **Music Assembly dashboard** (`music_assembler/api/app.py` — `_DASHBOARD_HTML` and `_LOGIN_HTML`) implements this system with:

- **Canvas:** Linen (`#fafffa`) full-bleed — no pure white panels
- **Display:** Playfair Display 300 (substitute for Editorial New) at hero + section `h2`
- **UI/body:** Inter 400/600 (substitute for TWK Lausanne)
- **Wordmark:** `Music` in Obsidian Ink + `Assembly` in Voltage
- **Primary CTA:** `.btn-voltage` — green fill, dual green shadow, uppercase — **only** on main actions (`Start job →`, login `Continue →`)
- **Secondary:** `.btn-ghost` — underline on hover, no fill (Refresh, Sign out, tabs, copy, cancel)
- **Cards:** Linen fill, 14px radius, **no border or shadow** — hierarchy from type scale
- **Accent ticks:** `h2::after` green 50×2px bars
- **Imagery:** thumbnails use `filter: grayscale(100%)` and 14px radius
- **Progress:** Voltage bar on Pollen track

When adding UI, use tokens from the **CSS Custom Properties** block above — Voltage is the only chromatic accent.
